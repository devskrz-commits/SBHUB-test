/* ==========================================
   1. CYBERPUNK ROBOT SECURITY LOGIN ENGINE
   ========================================== */
...
window.verifyLogin = function() {
  ...
  const card = document.querySelector(".login-popup-card");

  if (!pwdInput) return;

  const validUser = userInput ? (userInput.value.trim() === "sportsbook2026" || userInput.value.trim() === "sportsbookhub") : true;

  if (pwdInput.value === "sb2026" && validUser) {
    ...
    if (errEl) errEl.innerText = "ACCESS DENIED: Invalid Security Key";
...
/* --- ROBOT AVATAR HEAD & EYE SKELETON MOUSE TRACKING --- */
...
/* BRAND DIRECTORY TAB SWITCHER */
const brandDirectoryTabs = {
  ibet: [
...
window.switchDockTab = function(tabKey, btnEl) {
  ...
  const container = card.querySelector('#brandDirectoryLinksContainer');
  if (!container) return;
  container.innerHTML = '';

  const links = brandDirectoryTabs[tabKey] || [];
  links.forEach(item => {
    const a = document.createElement('a');
    a.className = 'dock-link-item';
    a.href = item.url;
    if (item.url !== '#') a.target = '_blank';
    a.innerHTML = `<span>${item.name}</span> <i class='bx bx-link-external'></i>`;
...
/* ==========================================
   4. FAIL-SAFE APPLICATION BOOTSTRAPPER
   ========================================== */

document.addEventListener("DOMContentLoaded", () => {
  try { window.checkRememberedSession(); } catch(e) { console.error("Session check error:", e); }
  try { initRobotInteractions(); } catch(e) { console.error("Robot init error:", e); }
  ...
  // Default brand directory tab view
  try { switchDockTab('ibet', document.querySelector('.dock-pill-btn')); } catch(e){}
